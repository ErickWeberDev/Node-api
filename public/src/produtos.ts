...
export function setupProductsInterface(actions?: ProductActions): void {
  document
    ...
    form?.addEventListener("submit", async (event) => {
      ...
      try {
        if (modalId === "add-product-modal") {
          ...
          if (!window.confirm(`Deseja deletar o produto de ID ${id}?`)) return;
...
function formatPrice(price: number | string): string {
  ...
  if (!Number.isFinite(numericPrice)) return "Preço indisponível";
...
export function renderProducts(products: Produto[]): void {
  const table = document.querySelector<HTMLElement>(
    "#products-view .table-card",
  );
  if (!table) return;
